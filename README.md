...
An invite used to pay the sender only. The operator who accepted it got nothing
for accepting, which made the invite a favour asked rather than an offer made —
and the click is the cheap part, the first session is what has to be bought.

The payout is split in two, and the split is the design:

| | Inviter | Invited operator |
| --- | --- | --- |
| On the link binding | — | +500 Intel, +2 chips |
| On qualifying | +5 chips, +5% passive Intel (cap +150%), +120 airdrop score | +5 chips, +200 Signal Points |

The welcome kit carries **no Signal Points on purpose**. SP feed the airdrop
score one for one, so paying them at signup would pay for creating an account
rather than for playing, and a farm of throwaway accounts would mint ranking out
of nothing. Intel and chips have to be spent inside the game before they move
the score at all.

The second half fires on the event that already paid the inviter — the invited
operator reaching level 3 with one signal call behind them. By then the account
has demonstrably played, so Signal Points are safe, and both sides are paid in
the same moment.

A device already tied to another account still binds — a shared family phone is
not broken — but collects no welcome kit: one phone accepting its own
invitations is the cheapest farm there is. The inviter's daily cap of ten
qualified referrals is unchanged, and a referral past it pays nothing and
announces nothing; the operator who played is paid either way, because their
reward does not depend on how busy their recruiter's day was.

Both sides are told. Notices queue on the player document and wait for a free
screen exactly like achievement unlocks, and the client acknowledges each one
(`/api/game/referral/ack`) so a reload does not celebrate it twice.
