...
export const REQUIRE_WALLET_FOR_ACTIVATION = String(process.env.REQUIRE_WALLET_FOR_ACTIVATION || 'false') !== 'false';

// Genesis activation is "played enough to count" — taps + one assessment. It is
// deliberately NOT gated on the wallet by default, so the first-1000 Genesis
// race stays about play. Set REQUIRE_WALLET_FOR_ACTIVATION=true to fold the
// wallet into it as well.
export function activationEligible(player) {
  ...
  const walletOk = !REQUIRE_WALLET_FOR_ACTIVATION || Boolean(player?.progression?.wallet?.address);
  return taps >= 25 && assessments >= 1 && walletOk;
}

// Airdrop eligibility is the stricter, Blum-style gate: real play AND a
// connected wallet. This is what the client shows on the airdrop/Network screen
// as the "connect your wallet to qualify" requirement.
export function airdropEligible(player) {
  const taps = Number(player?.progression?.signalEmpire?.scan?.taps || 0);
  const assessments = Number(player?.stats?.reconAttempts || 0);
  return taps >= 25 && assessments >= 1 && Boolean(player?.progression?.wallet?.address);
}
...
export function publicGrowthState(player) {
  ...
  return {
    ...
    activation: {
      ...
      walletConnected: Boolean(player?.progression?.wallet?.address),
      walletRequired: REQUIRE_WALLET_FOR_ACTIVATION,
      airdropEligible: airdropEligible(player)
    }
  };
}
