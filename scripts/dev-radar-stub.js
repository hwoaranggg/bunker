...
export function startRadarStub({ port = Number(process.env.RADAR_STUB_PORT || 3311), apiKey = process.env.XRADAR_GAME_API_KEY || STUB_API_KEY } = {}) {
  ...
  app.post('/api/game/trading/summary', guard, (_req, res) => {
    // Zeroed and unverified by default. Set STUB_TRADING_VERIFIED=true and,
    // optionally, STUB_TRADE_COUNT / STUB_TRADE_VOLUME to exercise the trading
    // conversion ladder locally.
    const verified = process.env.STUB_TRADING_VERIFIED === 'true';
    res.json({
      ok: true,
      verified,
      tradeCount: verified ? Number(process.env.STUB_TRADE_COUNT || 3) : 0,
      volumeUsd: verified ? Number(process.env.STUB_TRADE_VOLUME || 250) : 0
    });
  });
